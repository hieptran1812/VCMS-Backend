...
class SegDetectorRepresenter():
    def __init__(self, thresh=0.5, box_thresh=0.6, max_candidates=1000, unclip_ratio=2):
        self.min_size = 3
        self.thresh = thresh
    ...
    def __call__(self, batch, pred, is_output_polygon=False):
        ...
        for batch_index in range(pred.size(0)):
            height, width = batch['shape'][batch_index] # Retrieve h_origin and w_origin
    ...
    def get_mini_boxes(self, contour):
        try:
            ...
            if 0.0 < bounding_box[2] <= 45.0:
                bounding_box = tuple([bounding_box[0], bounding_box[1], 0.0])
            if 45.0 < bounding_box[2] < 90.0:
                bounding_box = tuple([bounding_box[0], bounding_box[1], 90.0])
    ...
    def box_score_fast(self, bitmap, _box):
        h, w = bitmap.shape[:2]
        box = _box.copy()
        x_min = np.clip(np.floor(box[:, 0].min()).astype(int), 0, w - 1)
        x_max = np.clip(np.ceil(box[:, 0].max()).astype(int), 0, w - 1)
        y_min = np.clip(np.floor(box[:, 1].min()).astype(int), 0, h - 1)
        y_max = np.clip(np.ceil(box[:, 1].max()).astype(int), 0, h - 1)
