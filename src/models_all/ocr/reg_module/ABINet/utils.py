...
class CharsetMapper(object):
    ...
    def _read_charset(self, filename):
        ...
        with open(filename, 'r') as fr:
            ...
            for line in lines:
                ...
                label, char = int(line[0]), line[1]
                charset[label + 1] = char
                last_label = label + 1
        charset[last_label + 1] = ' '
        return charset
    ...
    def get_labels(self, text, length=None, padding=True, case_sensitive=True):
        ...
        labels = []
        for char in text:
            if char in self.char_to_label.keys():
                labels.append(self.char_to_label[char])
            else:
                labels.append(0)
...
class Config(object):
    ...
    def __init__(self, config_path, host=True):
        ...
        with open('ABINet/configs/template.yaml') as file:
